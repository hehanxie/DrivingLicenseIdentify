...
Rect DriverLicense::getTopSideArea(Rect upperSideArea, float ratio)
{
...
Mat DriverLicense::locateKeyword(Rect rect, float widthOffsetRatio, float heightOffsetRatio, float widthRatio, float heightRatio)
{
	int width = rect.x;
	int height = rect.y;
	Point p1;
...
//	imshow("dilate image", dilateImage);
	...
	Mat cannyImage;
	Canny(dilateImage, cannyImage, 3, 9,3 );
